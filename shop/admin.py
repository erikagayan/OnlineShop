...
class MicroserviceUserAdmin(admin.ModelAdmin):
    ...
    def get_urls(self):
        ...
        custom_urls = [
            path(
                ...
                self.admin_site.admin_view(self.edit_user),
                name="edit-user",
            ),
        ]
    ...
    def get_object(self, request, object_id, from_field=None):
        token = request.session.get("auth_token") or request.headers.get(
            "Authorization"
        )
        if not token:
            return None
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        try:
            ...
            if not token:
                self.message_user(
                    request, "Authorization token is missing.", level="error"
                )
                return response
            ...
            response_data = requests.get(
                f"{settings.MICROSERVICE_USER_URL}/api/users/users/", headers=headers
            )
            if response_data.status_code != 200:
                self.message_user(
                    request,
                    f"Error fetching users: {response_data.status_code}",
                    level="error",
                )
                return response
    ...
    def save_model(self, request, obj, form, change):
        ...
        try:
            ...
            if response.status_code in [200, 204]:
                ...
                self.message_user(
                    request,
                    f"Error updating user: {response.status_code}",
                    level="error",
                )
        except Exception as e:
            self.message_user(request, f"Unexpected error: {e}", level="error")
    ...
    def delete_model(self, request, obj):
        ...
        try:
            ...
            if response.status_code == 204:
                ...
                self.message_user(
                    request,
                    f"Error deleting user: {response.status_code}",
                    level="error",
                )
        except Exception as e:
            self.message_user(request, f"Unexpected error: {e}", level="error")
            ...
            if form.is_valid():
                ...
                try:
                    ...
                    if response.status_code in [200, 201]:
                        ...
                        self.message_user(
                            request,
                            f"Error adding user: {response.status_code}",
                            level="error",
                        )
                except Exception as e:
                    self.message_user(request, f"Unexpected error: {e}", level="error")
    ...
    def edit_user(self, request, user_id):
        if request.method == "POST":
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.message_user(
                        request,
                        f"Error fetching user: {response.status_code}",
                        level="error",
                    )
                    return HttpResponseRedirect("..")
            except Exception as e:
        ...
        return self.render_change_form(
            request, context, add=False, change=True, form_url=form_url
        )
    ...
    def delete_users(self, request, queryset):
        for user in queryset:
            ...
            try:
                response = requests.delete(
                    f"{settings.MICROSERVICE_USER_URL}/api/users/users/{user.id}/",
                    headers=headers,
                )
                if response.status_code == 204:
                    self.message_user(
                        request, f"User {user.email} deleted successfully."
                    )
                else:
                    self.message_user(
                        request,
